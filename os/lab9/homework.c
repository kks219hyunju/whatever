...
void *wastemem(int a) {
  ...
  int *mallblock = NULL;
  long mallocnum = 1024*1024*1024; // 1 computer-million * user request

  printf("malloc'ing a pile of RAM....\n");
  mallblock = calloc(mallocnum, sizeof(int)); // of this architecture's integer
  if (mallblock <= 0) {
    char failmsg[128];
...
int main(int argc, char* *argv) //delcaring input arguments in main function
{
	...
	while((opt= getopt(argc,argv,"rmh")) != -1)
	{
		...
		if(hflag == 0 && rflag == 1 && optind < argc)
		{
		 do
		   {
			wastemem(mvalue);
		...
		else if(hflag == 1 && rflag == 0)
		{
		  ...
		  // help functoin
	        ...
	        {
		  printf("PASS IN ONLY ONE VARIABLE\n");
		}
